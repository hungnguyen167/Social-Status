...
ggplot() +
  ...
  geom_errorbar(aes(x = count, ymin = mod_b-(1.96*mod_se), ymax = mod_b+(1.96*mod_se)), color = "grey") +
  labs(#color = "Effect at 95% CI", 
    x = "Bayes Models Ordered by Moderation Coefficient (Y11)", y = "Moderation Coefficient")
```
...
df_ame_bayes_mod <- df_ame_bayes %>%
  arrange(mod_var, mod_b) %>%
  mutate(count = row_number())

df_ame_bayes_mod <- df_ame_bayes_mod %>%
...
```{r spec_by_mod_plot}
mcount1 = as.numeric(length(df_ame_mod$mod_b[df_ame_mod$mod_var == "A1_Corrupt_micro"])) # counter to make vertical bars
mcount2 = length(df_ame_mod$mod_b[df_ame_mod$mod_var != "A1_Corrupt_micro"])/2
...
fig4 <- df_ame_mod %>%
  ...
  geom_point(aes(x = count, y = mod_b_A1), color = "#73D055FF", 
             #shape = "|", 
             shape = 16,
             size = 2, show.legend =F) +
  geom_point(aes(x = count, y = mod_b_A2), color = "#287D8EFF", 
             #shape = "|", 
             shape = 16,
             size = 2, show.legend =F) +
  geom_point(aes(x = count, y = mod_b_B), color = "#453781FF", 
             #shape = "|", 
             shape = 16,
             size = 2, show.legend =F) +
  geom_vline(xintercept = mcount1, color = "grey", linetype = "dashed") +
  geom_vline(xintercept = mcount1+mcount2, color = "grey", linetype = "dashed") +
  ...
  annotate(geom = "text", x = mcount1/2, y = 0.12, label = "Corrupt occupational\ncareers (ISSP)", color = "#73D055FF", size = 3) +
  annotate(geom = "text", x = mcount1+(mcount2/2), y = 0.12, label = "Corruption perceptions\n(CPI)", color = "#287D8EFF", size = 3) +
  annotate(geom = "text", x = mcount1+mcount2+(mcount2/2), y = 0.12, label = "Lack of Trust\n in Government (IVS)", color = "#453781FF", size = 3) +
