...
df_ame_mod <- df_ame_mod %>%
  mutate(mod_b_lib = ifelse(mod_var == "A_Liberal_values", mod_b, NA),
...
```{r spec_by_mod_plot}
agg_png(here::here("results","Fig4_top.png"), height = 600, width = 1200, res = 144)
fig4 <- df_ame_mod %>%
  ggplot() +
  #geom_errorbar(aes(ymin = lb, ymax = ub), color = "thistle1") +
  geom_errorbar(aes(x = count, ymin = mod_b-(2.576*mod_se), ymax = mod_b+(2.576*mod_se)), color = "grey", width = 0) +
  geom_vline(xintercept = length(df_ame_mod[df_ame_mod$mod_var == "C_Corruption_percept",1])-0.5, color = "grey", linetype = "dashed") +
  geom_vline(xintercept = (2*length(df_ame_mod[df_ame_mod$mod_var == "C_Corruption_percept",1]))-0.5, color = "grey", linetype = "dashed") +
  ...
  annotate(geom = "text", x = 56, y = 0.13, label = "Liberal Values\nhigh abstraction", color = "#73D055FF") +
  annotate(geom = "text", x = 168, y = 0.13, label = "Lack of Trust\nmedium abstraction", color = "#287D8EFF") +
  annotate(geom = "text", x = 280, y = 0.13, label = "Corruption Perceptions\nlow abstraction", color = "#453781FF") +
  theme_classic() +
    theme(
...
knitr::include_graphics(here::here("results","Fig4_top.png"))
```
...
```
```{r}
agg_png(here::here("results","Fig4.png"), height = 1000, width = 1200, res = 144)
