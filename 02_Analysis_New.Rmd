...
Soeren? - update graph to include multiple slopes (in different colors) for within country regressions at different time points

Question - the inclarge question was not asked in the environment module I guess

Nate - get newer measure of socx from OECD
...
### Multilevel Models
```{r mlms}
M00 <- lmer(incdiff ~ 1 + (1 | iso3c), data = df_na)
M01 <- lmer(incdiff ~ 1 + inclarge + (1 + inclarge | iso3c), data = df_na)
M02 <- lmer(incdiff ~ 1 + inclarge*socx + (1 + inclarge | iso3c), data = df_na)
M03 <- lmer(incdiff ~ 1 + inclarge*gini_disp + (1 + inclarge | iso3c), data = df_na)

tab_model(M00, M01, M02, M03, show.ci = F, p.style = "stars")
