...
Data first setup in 01.1 and 01.2 files.
...
mmmPOOLED <- ggplot(data=df, aes(x=reduce_large, y=reduce)) +
...
for (j in 1:length(unique(fitted_models$iso3c))){
  dfx <- df_na %>% # subset data to only that country and remove missing
    subset(iso3c == fitted_models$iso3c[j]) %>% # this is just a shortcut to cycle through each country
    subset(!is.na(reduce) & !is.na(reduce_large))
  
  plot1 <- ggplot(data = dfx, aes(x = reduce_large, y = reduce, group = wave, color=as.factor(wave))) +
...
df <- df %>%
  ...
  mutate(reduce_b = mean(reduce, na.rm = T),
         reduce_large_b = mean(reduce_large, na.rm = T)) %>%
  ungroup() %>%
  # calculate the mean-centered values by subtracting the means
  mutate(reduce_w = reduce - reduce_b,
         reduce_large_w = reduce_large - reduce_large_b)

```

Now we can see the within- and between-variance for each level for the variable *reduce* and then we can see how much of this is explained by *reduce_large*. To do this we need to predict each level's outcomes. 
...
# between-regression
df_b <- df %>%
  dplyr::select(iso3c, reduce_b, reduce_large_b) %>%
  group_by(iso3c) %>%
  summarise_all(mean, na.rm = T)

m02_b <- lm(reduce_b ~ reduce_large_b, data = df_b)

# add predicted values to df_b
df_b$reduce_b_hat <- predict(m02_b, newdata = df_b)
  
# within-regression
df_w <- df %>% # need to remove missing cases for predict to work
  subset(!is.na(reduce_w) & !is.na(reduce_large_w))

# add predicted values to df_w
m02_w <- lm(reduce_w ~ reduce_large_w, data = df_w)

df_w$reduce_w_hat <- predict(m02_w, data = df_w)

# add predicted values to df
df_t <- df %>%
  subset(!is.na(reduce) & !is.na(reduce_large))

df_t$reduce_hat <- predict(m01, data = df_t)
  
# variances
reduce_b_var <- round(var(df_b$reduce_b, na.rm = T),3) # between variance
reduce_w_var <- round(var(df$reduce_w, na.rm = T),3) # within variance
reduce_var <- round(var(df$reduce, na.rm = T),3) # total variance
reduce_b_hat_var <- round(var(df_b$reduce_b_hat, na.rm = T),3) # predicted values variance
reduce_w_hat_var <- round(var(df_w$reduce_w_hat, na.rm = T),3)
reduce_hat_var <- round(var(df_t$reduce_hat),3)

# residual variance 
reduce_b_var_h <- round((reduce_b_var - reduce_b_hat_var),3)
reduce_w_var_h <- round((reduce_w_var - reduce_w_hat_var),3)
reduce_var_h <- round((reduce_var - reduce_hat_var),3)
```
...
| ----- | --- | --- | --- |
| reduce total variance $Y_{ij}$ | `r reduce_var` | -- | -- | standard var calculation |
| reduce between-variance $Y_{j}$ | `r reduce_b_var` | -- | -- | *ibid* |
| reduce within-variance $Y_{i}$ | `r reduce_w_var` | -- | -- | *ibid* |
| after adding *reduce_large* as predictor | | | |
| $\sigma^2_{ij}$ of $\hat{Y}_{ij}$ (total) | `r reduce_hat_var` | `r paste(round(reduce_hat_var/reduce_var,3)*100,"%")` | `r round(summary(m01)$r.squared,3)` | standard $R^2$ |
| $\sigma^2_{j}$ of $\hat{Y}_{j}$ (between) | `r reduce_b_hat_var` | `r paste(round(reduce_b_hat_var/reduce_b_var,3)*100,"%")` | `r round(summary(m02_b)$r.squared,3)` | **Eq. 13** in Snijders and Bosker |
| $\sigma^2_{i}$ of $\hat{Y}_{i}$ (within) | `r reduce_w_hat_var` | `r paste(round(reduce_w_hat_var/reduce_w_var,3)*100,"%")` | `r round(summary(m02_w)$r.squared,3)` | ** Eq. 12** in Snijders and Bosker |
...
The rho for the ISSP data we are working with therefore suggests a multilevel model is a good idea because it is `r round(reduce_b_var/reduce_var,3)`. In other words, `r paste0(round(reduce_b_var/reduce_var*100,3),"%")`  of the total variance occurs between countries.
