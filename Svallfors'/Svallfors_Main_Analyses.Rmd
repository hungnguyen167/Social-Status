...
In 2003, Linos and West published a follow up to Svallfors’ (1997) study. The expanded his statistical model to include years of education, income and marital status and demonstrated that there were significant differences across countries in the linkage between occupational classes and attitudes toward redistribution. They find a difference by martial status in redistributive preferences but show that the gap disappears or can be explained by other factors in "universalist" regimes (i.e. social democractic institutional arrangements). The find that insider/outsider cleavages are most pronounced in conservative regimes and that class matters more in liberal regimes. 

Criticisms: they have not dealt with marriage and gender together, there is probably an interaction. Also, they treat 'outsiders' as the unemployed. This does not follow the dualist labor market literature which characterizes 'insiders' as those with full-time positions, a permanent or long-term contract and strong social security versus 'outsides' as those with often less than full-time (or full-time, but with several part-time jobs), precarious contracts (hourly, short term) and less or no social security. Class should not matter more in liberal regimes, this seems to contradict the ideas of individualism and neoliberalism which favors values of limited government and strong opposition to free riders (or ethnic-based attitudes).
...
#we use Erik Bihagen's translation of NYK into ISCO88 [introduces error/not perfectly translatable] http://www.camsis.stir.ac.uk/occunits/SwedishNYK8590majgpsv1.sps
...
```{r income, echo=T}
df$hhinc <- coalesce(df$cdn116,df$s117,df$usa116,df$n116,df$a116,df$aus116,df$nz116,df$d116,df$i116)
...
df$cntry = car::recode(df$v3,recodes = '10=752; 9=578; 2=276; 1=36; 6=40; 16=554; 17=124; 5=840; 3:4=NA; 7=NA; 8=380; 11:15=NA; 18=NA')
...
#Full-time=1 (1 = full-time); part-time (2,3 = part-time or less-than), dependent (5,6,7,9 = unemployed, student, retired perm. disabled), not in lf or unknown (4,8,10, 99 = family care, homemaker, other, missing)
df$workstat = car::recode(df$v104,recodes = '1=1; 2:3=2; 5:6=3; 7=3; 9=3; 4=4; 8=4; 10=4; 99=4')

df$s103 <- ifelse(is.na(df$s103), df$cdn103, df$s103)
df$v102 <- ifelse(is.na(df$v102), df$s103, df$v102)

# replace part-time/full-time based on hours worked per week
df$workstat = ifelse(df$v105>35 & df$v105<96, 1, df$workstat)
df$workstat = ifelse(df$v105>1 & df$v105<31, 2, df$workstat)
...
```{r oprobit, echo=T}
# At first we exclude Italy to be directly comprable to the Svallfors study.
df_list$cntry <- ifelse(df_list$cntry==380, NA, df_list$cntry)
df_list <- df_list[complete.cases(df_list$cntry),]
...
# Jobs
plot(xv, yv, type="l", lty=1, xlab="", ylab="Probability", main="Response Distribution: Government should provide jobs", sub="dotted lines = cutpoints, assigining responses to a normal distribution")
...
text(-3.5, 0.035, "strongly", cex=0.6, col="blue")
text(-3.5, 0.015, "disagree", cex=0.6, col="blue")
text(-1.7, 0.035, "disagree", cex=0.6, col="blue")
text(-0.45, 0.035, "neutral", cex=0.6, col="blue")
text(1, 0.035, "agree", cex=0.6, col="blue")
text(3.1, 0.035, "strongly", cex=0.6, col="blue")
text(3.1, 0.015, "agree", cex=0.6, col="blue")

# Income Differences
plot(xv, yv, type="l", lty=1, xlab="", ylab="Probability", main="Response Distribution: Government should reduce income differences", sub="dotted lines = cutpoints, assigining responses to a normal distribution")
...
text(-3.8, 0.035, "strongly", cex=0.6, col="blue")
text(-3.8, 0.015, "disagree", cex=0.6, col="blue")
text(-2.2, 0.035, "disagree", cex=0.6, col="blue")
text(-0.8, 0.035, "neutral", cex=0.6, col="blue")
text(0.5, 0.035, "agree", cex=0.6, col="blue")
text(2.96, 0.035, "strongly", cex=0.6, col="blue")
text(2.96, 0.015, "agree", cex=0.6, col="blue")

# Basic Income
plot(xv, yv, type="l", lty=1, xlab="", ylab="Probability", main="Response Distribution: Government should provide a basic income", sub="dotted lines = cutpoints, assigining responses to a normal distribution")
...
text(-3.6, 0.035, "strongly", cex=0.6, col="blue")
text(-3.6, 0.015, "disagree", cex=0.6, col="blue")
text(-2.05, 0.035, "disagree", cex=0.6, col="blue")
text(-0.85, 0.035, "neutral", cex=0.6, col="blue")
text(0.45, 0.035, "agree", cex=0.6, col="blue")
text(3, 0.035, "strongly", cex=0.6, col="blue")
text(3, 0.015, "agree", cex=0.6, col="blue")
```

We can see that the distribution when taking the three questions and combining them with equal weighting and then rounding to the nearest value, we get a slightly skewed normal distribution. It looks like what we would expect form attitude research.
...
dfp <- df
dfp$edyrs <- dfp$v102
dfp$edyrs2 <- dfp$edyrs^2
dfp$cntry <- car::recode(dfp$cntry,recodes='380=NA')
dfp <- dfp[complete.cases(dfp$cntry),]
dfp$cntry <- as.factor(dfp$cntry)

# predict probabilities

dfj <- as.data.frame(predict(ologit_jobs, dfp, type="prob"))
colnames(dfj)[1] <- "jobs1"
colnames(dfj)[2] <- "jobs2"
colnames(dfj)[3] <- "jobs3"
colnames(dfj)[4] <- "jobs4"
colnames(dfj)[5] <- "jobs5"

dfi <- as.data.frame(predict(ologit_incd, dfp, type="prob"))
colnames(dfi)[1] <- "incd1"
colnames(dfi)[2] <- "incd2"
colnames(dfi)[3] <- "incd3"
colnames(dfi)[4] <- "incd4"
colnames(dfi)[5] <- "incd5"

dfb <- as.data.frame(predict(ologit_basic, dfp, type="prob"))
colnames(dfb)[1] <- "basic1"
colnames(dfb)[2] <- "basic2"
colnames(dfb)[3] <- "basic3"
colnames(dfb)[4] <- "basic4"
colnames(dfb)[5] <- "basic5"

# predict response

dfjr <- as.data.frame(predict(ologit_jobs, dfp, type="class"))
colnames(dfjr)[1] <- "jobsR"


dfir <- as.data.frame(predict(ologit_incd, dfp, type="class"))
colnames(dfir)[1] <- "incdR"


dfbr <- as.data.frame(predict(ologit_basic, dfp, type="class"))
colnames(dfbr)[1] <- "basicR"

# convert factor response to numeric
dfir$incdR <- as.numeric(dfir$incdR)
dfjr$jobsR <- as.numeric(dfjr$jobsR)
dfbr$basicR <- as.numeric(dfbr$basicR)

# bind predicted values
dfp <- cbind.data.frame(dfp, dfj, dfjr, dfi, dfir, dfb, dfbr)

# get probabilities by response to v59 (jobs)
jm1 <- aggregate(dfp$jobs1, by=list(dfp$v59), FUN = mean, na.rm = T)
jm2 <- aggregate(dfp$jobs2, by=list(dfp$v59), FUN = mean, na.rm = T)
jm3 <- aggregate(dfp$jobs3, by=list(dfp$v59), FUN = mean, na.rm = T)
jm4 <- aggregate(dfp$jobs4, by=list(dfp$v59), FUN = mean, na.rm = T)
jm5 <- aggregate(dfp$jobs5, by=list(dfp$v59), FUN = mean, na.rm = T)

# get probabilities by response to v59 (jobs)
im1 <- aggregate(dfp$jobs1, by=list(dfp$v59), FUN = mean, na.rm = T)
im2 <- aggregate(dfp$jobs2, by=list(dfp$v59), FUN = mean, na.rm = T)
im3 <- aggregate(dfp$jobs3, by=list(dfp$v59), FUN = mean, na.rm = T)
im4 <- aggregate(dfp$jobs4, by=list(dfp$v59), FUN = mean, na.rm = T)
im5 <- aggregate(dfp$jobs5, by=list(dfp$v59), FUN = mean, na.rm = T)

# get mean class by response 
jmr <- aggregate(dfp$jobsR, by=list(dfp$v59), FUN = mean, na.rm = T)
imr <- aggregate(dfp$incdR, by=list(dfp$v57), FUN = mean, na.rm = T)
bmr <- aggregate(dfp$basicR, by=list(dfp$v62), FUN = mean, na.rm = T)

tjobo <- cbind(jmr, imr, bmr)

kable(tjobo)
```
Results so far, 

For the Jobs and Income Differences questions, those who selected "Can't choose" are predicted to have the highest response score on average (we should check the N).

For the Basic Income question, the 'Can't choose' are predicted closer to the "agree" category.

But there is a lot of missing data due to so many control variables. **We should try alternative models with fewer**.
