...
pacman::p_load("dplyr","tidyverse","knitr","haven","car","plyr","data.table","expss","htmlTable","magrittr","ltm","questionr","survey","remotes","stargazer","lavaan","ltm", "tidylog", "kableExtra","lm.beta","MplusAutomation","MASS")
...
```{r income, echo=T}
df$hhinc <- coalesce(df$cdn116,df$s116,df$usa116,df$n116,df$a116,df$aus116,df$nz116,df$d116)
...
df$female = car::recode(df$v99,recodes = '2=0; 9=NA')
#trim age to 80
...
SVallfors does not offer an argumentation. Linos and West (2003) argue for a continuous variable. empirical question by comparing the Svallfors' distribution versus a more 'classical' attitudes distribution.
...
### Table 1. The Distribution of Svallfors' Scale (Additive, thus assigning equal weight to each of the three variables)

Distribution is not normal, also does not appear 'continuous' as this would imply a normal distribution or something smooth or curved. Figure 1 shows a strong skew between those who answered agree or strongly agree to all 3 questions, and everyone else.

```{r govindex dist}
ggplot(socIn92recode, aes(govIndex)) +
  geom_bar(fill = "black") + 
  ggtitle("Figure 1. Svallfors Collapsed Categories as Additive Index")

```
```{r indivplots, include = T}

socIn92m$v57 <- car::recode(socIn92m$v57,recodes = '1=5; 2=4; 3=3; 4=2; 5=1; 8=8; 9=NA')
socIn92m$v59 <- car::recode(socIn92m$v59,recodes = '1=5; 2=4; 3=3; 4=2; 5=1; 8=8; 9=NA')
socIn92m$v62 <- car::recode(socIn92m$v62,recodes = '1=5; 2=4; 3=3; 4=2; 5=1; 8=8; 9=NA')
...
socIn92m$v57 <- as.factor(socIn92m$v57)
socIn92m$v59 <- as.factor(socIn92m$v59)
socIn92m$v62 <- as.factor(socIn92m$v62)
socIn92m<-socIn92m[complete.cases(socIn92m$v57),]
socIn92m<-socIn92m[complete.cases(socIn92m$v59),]
socIn92m<-socIn92m[complete.cases(socIn92m$v62),]

# Why should we expect a normal distribution? This is how the raw data look
ggplot(socIn92recode, aes(v57)) +
  geom_bar(fill = "black") + 
  scale_x_discrete("Original Response Choices (can't choose/non-response omitted)", labels = c("Strongly disagree","Disagree","Neutral","Agree" ,"Strongly agree")) + 
  ggtitle("Gov. Provide Jobs for Everyone that Wants One")
```

```{r jobsm, include=T}
ggplot(socIn92m, aes(v57)) +
  geom_bar(fill = "black") + 
...
ggplot(socIn92recode, aes(scale)) +
  geom_bar(fill = "black") + 
  scale_x_discrete("*Averaged Continuous Response Choice, 3 questions", labels = c("Strongly disagree*","Disagree*","Neutral*" ,"Agree*" ,"Strongly agree*")) + 
