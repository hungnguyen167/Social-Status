...
temp <- select(socin2, AT_RIN87:US_RIN09)
...
df <- select(df, c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, union, incdiff))
...
env93 <- select(env93,c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, union, incdiff))
...
# Select relevant variables 
env00 <- select(env00,c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, union, incdiff)) # single parent is temporarily out
...
temp <- select(env10, ar_rinc:za_rinc)
centered.temp <- scale(temp, center = TRUE, scale = TRUE)
env10$incZ <- coalesce(centered.temp)
# Select relevant variables 

env10 <- select(env10,c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, union, incdiff))
```
