...
1. Variable 'child' is not working
2. Can you please add the following countries using the single-country files and merging them in? 
  a. Social Inequality 1992 - add Spain manually
  b. Social Inequality 1999 - add Denmark and Brazil manually
  c. Social Inequality 2009 - add Netherlands manually
3. I had to change yoru dplyr recode commands, they didn't run on my machine, so they are now 'car::recode' for the variable country
4. I added a variable 'wave' which indicates the year associated with the survey
5. I changed the variable 'year' to reflect the actual year when the survey was fielded in each country - sometimes it is not the same as 'wave', but having the year accurate as possible makes it better for merging in macro data
6. Made a variable 'reduce' that is = to 'incdiff' but has 9 recoded into NA and has 8 recoded into neutral, also reversed the coding so that higher values = more agreement
...
```{r socin, include = FALSE, warning=F,message=F}
## General variables
colnames(socin) <- tolower(colnames(socin))

socin <- socin %>% 
  mutate(
     cntry = v5, #already in ISC alpha-3 coding
     year = v4,
     ...
     married = ifelse(cohab == 2, 1, married), # recode any life-partner to married
     educyrs = case_when(                 # years in school, trim at 25 years
       .$educyrs > 25 & .$educyrs < 90 ~ 26, # 26 = 25+ years in school
       .$educyrs %in% c(95,96) ~ NA_real_,
...
## Income.
### Without comparable categories in the surveys across countries
### we take each country-specific earnings measure and standardize it 
### within countries

# Respondent income
temp <- dplyr::select(socin2, AT_RIN87:US_RIN09)
centered.temp <- as.data.frame(scale(temp, center = TRUE, scale = TRUE)) # standardize
socin <- cbind(socin,centered.temp)
...
# trim extreme values
socin$rincZ = ifelse(socin$rincZ > 10, 10, socin$rincZ)
...
# trim extreme values
socin$hincZ = ifelse(socin$hincZ > 10, 10, socin$hincZ)
...
# socin$au_occ2 <- isco88conversion::anzscoToISCO(socin$au_occ)
...
# From US87 to USt
socin$us_occ <- car::recode(socin$us_occ, recodes = "1=23;2=43;3=64;4=64;5=64;6=44;10=48;11=75;12=55;13=19;14=57;  
...
socin$us_occ <- car::recode(socin$us_occ, recodes = "003=1110;004=1120;005=1120;006=1120;
...
socin$jp_occ <- car::recode(socin$jp_occ, recodes = "7 = 2265; 8 = 2265; 5 = 9322; 4 = 7233; 3 = 5130; 6 = 4115; 1 = 6130; 2 = 7233; 
                              9  = NA; 10= NA; 11 = NA")
...
socin$se_occ <- car::recode(socin$se_occ, recodes = "1 = 2141; 2 = 2143; 3 = 2144; 4 = 2145; 5 = 3116; 6 = 3117; 7 = 2142; 8 = 2148; 9 = 3119; 12 = 3211; 13 = 3211; 14 = 2114; 15 = 2112; 16 = 2113; 19 = 3111; 21 = 2211; 22 = 3213; 23 = 3213; 29 = 2211; 30 = 1210; 31 = 2310; 32 = 2320; 33 = 2331; 34 = 2320; 35 = 2359; 36 = 2320; 37 = 2352; 39 = 2359; 41 = 2460; 49 = 3480; 51 = 2421; 52 = 2429; 53 = 2429; 
...
df <- dplyr::select(df, c(cntry, year, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, union, sparent, wrkhrs, wrkst, incdiff))
...
## Data transformation - Environment 1993


```{r env93, include = FALSE}
env93 <- env93 %>%
  mutate(
    # cntry = dplyr::recode(cntry, `1` = 36, `2` = 276, `3` = 276, `4` = 826, `5` = 826, `6` = 840, `8` = 348, `9` = 380,`10` = 372, `11` = 528, `12` = 578, `13` = 203, `14` = 705, `15` = 616, `16` = 100, `17` = 643, `18` = 554, `19` = 124, `20` = 608, `21` = 376, `22` = 392, `23` = 724), # Nate runs this and it does not recode
    cntry = car::recode(v3, recodes = '"1" = 36; "2" = 276; "3" = 276; "4" = 826; "5" = 826; "6" = 840; "8" = 348; "9" = 380;"10" = 372; "11" = 528; "12" = 578; "13" = 203; "14" = 705; "15" = 616; "16" = 100; "17" = 643; "18" = 554; "19" = 124; "20" = 608; "21" = 376; "22" = 392; "23" = 724'),
    year = 1993,
    female = v200 - 1,
    age = v201,
    incdiff = v6,
...
# It's a little problematic here because in GB or AUS, they cut at 14 and 20     
    educyrs = case_when(
       educyrs > 25 & educyrs < 90 ~ 26, 
       educyrs %in% c(95:98) ~ NA_real_,
...
env93 <- env93 %>%
  mutate(
    isco68 = as.numeric(v214),
    isco88 = as.numeric(v214)
  )
env93$isco68 <- occupar::isco68to88(isco68 = env93$isco68,display.nas = TRUE)
env93[which(env93$cntry %in% c(1, 10, 14, 15, 17)),]$isco68 <- NA
env93[which(!(env93$cntry %in% c(1, 10, 14, 15, 17))),]$isco88 <- NA


env93$isco88 <- coalesce(env93$isco68,env93$isco88)

# ISCO88 to EGP

env93$egp <- occupar::isco88toEGP(isco88 = env93$isco88, n.employees = env93$v221, self.employed = env93$v222)
...
# ISEI
env93$isei <- occupar::isco88toISEI92(isco88 = env93$isco88)
...
env93 <- env93 %>%
  group_by(cntry) %>%
  mutate(
    v231 = ifelse(v231 > 200000, 200000, v231), # trim extreme values
    rincZ = scale(v231, center = TRUE, scale = T),
  ) %>%
  ungroup()
...
env93 <- env93 %>%
  group_by(cntry) %>%
  mutate(
    v253 = ifelse(v253 > 300000, 300000, v253), # trim extreme values
    hincZ = scale(v253, center = TRUE, scale = T),
  ) %>%
  ungroup()

# Select relevant variables 

env93 <- dplyr::select(env93,c(cntry, year, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, union, sparent, wrkhrs, wrkst, incdiff))
...
```{r env00, include = FALSE}
env00 <- env00 %>%
  mutate(
    cntry = car::recode(v3, recodes = '"1" = 36; "2" = 276; "3" = 276; "4" = 826; "5" = 826; "6" = 840; "7" = 40; "8" = 348; "9" = 380; "10" = 372; "11" = 528; "12" = 578; "13" = 752; "14" = 203; "15" = 705; "16" = 616; "17" = 100; "18" = 643; "19" = 554; "20" = 124; "21" = 608; "22" = 376; "24" = 392; "25" = 724; "26" = 428; "30" = 620; "31" = 152; "32" = 208; "33" = 756; "37" = 246; "38" = 484'),
    year = 2000,
    female = v200 - 1,
    age = v201,
    incdiff = v6,
    ageC = ifelse(age > 75, 75, age), # trim at 75  
    age = case_when(     # by categories
       age %in% c(16:24) ~ 1,
       age %in% c(25:34) ~ 2,
       age %in% c(35:44) ~ 3,
       age %in% c(45:54) ~ 4,
...
env00$egp <- occupar::isco88toEGP(isco88 = env00$v233, n.employees = env00$v237, self.employed = env00$v236)
...
env00 <- env00 %>%
  group_by(cntry) %>%
  mutate(
    rincZ = scale(v240, center = TRUE, scale = TRUE)
  ) %>%
  ungroup()
...
env00 <- env00 %>%
  group_by(cntry) %>%
  mutate(
    hincZ = scale(v241, center = TRUE, scale = TRUE)
  ) %>%
  ungroup()

# Select relevant variables 
env00 <- dplyr::select(env00,c(cntry, year, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, union, sparent, wrkhrs, wrkst, incdiff)) 
```


## Data transformation - Environment 2010
```{r env10, include = FALSE}
...
env10 <- env10 %>%
  mutate(
    ...
    educyrs = case_when(  # how to deal with 95 and 96?
       educyrs > 25 & educyrs < 90 ~ 26, 
       educyrs == 95 ~ 8,
       educyrs == 96 ~ 14,
       educyrs %in% c(97:98) ~ NA_real_,
       TRUE ~ as.numeric(educyrs)
    ),
    child = case_when(
...
# income
### Without comparable categories in the surveys across countries
### we take each ordered scale and standardized it by country
### in this dataset, missings are not recoded
### missing >= 999990

# R income
temp <- dplyr::select(env10, ar_rinc:za_rinc)
temp[temp >= 999990] <- NA
centered.temp <- as.data.frame(scale(temp, center = TRUE, scale = TRUE))
#dimension problems arose here, had to alter the code
centered.temp$rincZ <- rowMeans(centered.temp, na.rm=T)
centered.temp$rincZ <- ifelse(centered.temp$rincZ=="NaN",NA,centered.temp$rincZ)
centered.temp <- dplyr::select(centered.temp, rincZ)
# trim extremes
centered.temp$rincZ <- ifelse(centered.temp$rincZ > 10, 10, centered.temp$rincZ)
env10 <- cbind(env10,centered.temp)
...
centered.temp <- as.data.frame(scale(temp, center = TRUE, scale = TRUE))
#dimension problems arose here, had to alter the code
centered.temp$hincZ <- rowMeans(centered.temp, na.rm=T)
centered.temp$hincZ <- ifelse(centered.temp$hincZ=="NaN",NA,centered.temp$hincZ)
centered.temp <- dplyr::select(centered.temp, hincZ)
# trim extremes
centered.temp$hincZ <- ifelse(centered.temp$hincZ > 10, 10, centered.temp$hincZ)
env10 <- cbind(env10,centered.temp)


# Select relevant variables 

env10 <- dplyr::select(env10,c(cntry, year, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, union, sparent, wrkhrs, wrkst, incdiff))
```


## Merge environment data sets and social inequality cumulative data set


```{r merge, include = FALSE}
env <- plyr::rbind.fill(env93,env00)
# I had to change these to a normal 'rbind' command because rbind.fill gave "dim" error
env <- rbind(env,env10)
df <- rbind(env,df)

# Make a year of survey variable

df <- df %>%
  mutate(wave = year,
         year = ifelse(wave == 1987 & cntry == 40, 1988, 
                       ifelse((wave == 1992 & cntry == 36) | (wave == 1992 & cntry == 40) | (wave == 1992 & cntry == 100), 1993, 
                              ifelse(wave == 1992 & cntry == 752, 1991, 
                                     ifelse((wave == 1999 & cntry == 40) |(wave == 1999 & cntry == 152) | (wave == 1999 & cntry == 276) | (wave == 1999 & cntry == 840), 2000, 
                                            ifelse((wave == 1999 & cntry == 348) | (wave == 1999 & cntry == 705), 1998, 
                                                   ifelse((wave == 1999 & cntry == 372) | (wave == 1999 & cntry == 703), 1998, 
...
df <- dplyr::select(df, cntry, year, wave, cyear, everything())
...
df$incdiff <- ifelse(df$incdiff == 9, NA, df$incdiff)
df$reduce <- car::recode(df$incdiff, '1 = 5; 2 = 4; 3 = 3; 8 = 3; 4 = 2; 5 = 1')
# made 'i don't know' into 'neutral' for now
...
# Descriptives / Coding-Validation
...
```{r corrs}
df_cor <- dplyr::select(df, reduce, ageC, female, educyrs, married, rincZ, hincZ, isei, union, sparent, wrkhrs, wrkst)

cor1 <- cor(df_cor, use = "pairwise.complete.obs")
