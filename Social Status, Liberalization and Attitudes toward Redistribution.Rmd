...
1. Variable 'child' has too many NA, do you know why? Can you fix it? It should have none, or almost none.
...
3. I had to change your dplyr recode commands, they didn't run on my machine, so they are now 'car::recode' instead for the variable country
...
7. Recoded 'wrkst' so that they matched across all surveys (not just full-time, part-time, less than part time). Env00 uses a slightly different list than the others
...
socin <- socin %>% 
  mutate(
     ...
     educyrs = case_when(                 # years in school, trim at 25 years
       ...
       .$educyrs == 95 ~ 8, # adjust for still in school
       .$educyrs == 96 ~ 13,
...
# fix to match env10 module
env93$wrkst <- ifelse(env93$wrkst == 4, 8, ifelse(env93$wrkst == 99 | is.na(env93$wrkst), 10, env93$wrkst))
...
# fix to match env10 module
env00$wrkst <- ifelse(env00$wrkst == 4, 8, ifelse(env00$wrkst == 99 | is.na(env00$wrkst), 10, env00$wrkst))
...
env10 <- env10 %>%
  mutate(
    ...
    wrkst = case_when(
      mainstat == 1 & wrkhrs > 34 ~ 1, # full
      mainstat == 1 & (wrkhrs > 14 & wrkhrs < 35) ~ 2, # part
      mainstat == 1 & wrkhrs < 15 ~ 3, # less than part
      mainstat == 3 ~ 6, # education
      mainstat == 4 ~ 6, # apprentice/trainee
      mainstat == 5 ~ 9, # perm sick/disabled
      mainstat == 6 ~ 7, # retired
      mainstat == 7 ~ 8, # housework & helping family member
      mainstat == 8 | is.na(mainstat) ~ 10) # other
  ) 
...
# clean up
rm(centered.temp, jp, socin2, temp)

# save

save.image(file = paste0(wd,"socstat.Rdata"))
```
