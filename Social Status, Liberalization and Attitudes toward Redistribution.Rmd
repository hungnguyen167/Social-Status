...
pacman::p_load("tidyverse","knitr","haven","car","plyr","e1071","stargazer","lavaan","kableExtra","MASS","glm.predict","brant", "semTools", "tidylog")
...
# ------- #
# Australia 1987 (waiting for response)
...
socin <- dplyr::select(socin, cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff)
...
dk99 <- dplyr::select(dk99, cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff)
...
ie99 <- dplyr::select(ie99, cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff)
...
nl09 <- dplyr::select(nl09, cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff)
...
env93 <- env93 %>%
  group_by(cntry) %>%
  mutate(
    incZ = scale(v231, center = TRUE, scale = T),
  )
...
env93 <- dplyr::select(env93,c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff))
...
# Select relevant variables 
env00 <- dplyr::select(env00,c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff)) 
```


## Data transformation - Environment 2010
```{r env10, include = TRUE}
...
env10 <- env10 %>%
  mutate(
    ...
    wrkst = case_when(
      ...
      mainstat == 7 & work == 1 ~ 4, # helping a family member 
      mainstat == 2 ~ 5,
      mainstat %in% c(3,4) ~ 6,
      mainstat == 6 ~ 7,
      mainstat == 7 & (work != 1 | (married == 1 & spwork == 1)) ~ 8, # housewife/houseman
      mainstat == 5 ~ 9,
...
# Select relevant variables 
env10 <- dplyr::select(env10,c(cntry, year, age, ageC, female, educyrs, degree, child, married, incZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff)) 

```
## Merge environment data sets and social inequality cumulative data set and save 

```{r mergesave, include = TRUE}
