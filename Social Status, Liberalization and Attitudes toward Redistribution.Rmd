...
centered.temp <- as.data.frame(scale(temp, scale = TRUE)) # standardizes
isspCUM <- cbind(isspCUM,centered.temp)
isspCUM$incZ = rowMeans(isspCUM[,which(names(isspCUM)=="AT_RIN87"):which(names(isspCUM)=="US_RIN09")], na.rm = T)
isspCUM$incZ = ifelse(isspCUM$incZ=="NaN",NA,isspCUM$incZ)

## 

# Think about ISCED for education (see Schneider's work)
