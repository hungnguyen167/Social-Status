...
```{r bayesame}
model_list_bayes <- mget(ls(pattern="M_b"))

   df_ame_bayes <- as.data.frame(matrix(nrow = length(model_list_bayes), ncol = 3))

colnames(df_ame_bayes) <- c("Model", "mod_b", "mod_se")

model_names <- names(model_list_bayes)

for (m in 1:length(model_list_bayes)) {
  
  #mn <- as.name(model_names[m])
  
  df_ame_bayes[m,1] <- paste(as.character(model_list_bayes[[m]][["formula"]][["formula"]]), sep = "", collapse = "")
  ...
  df_ame_bayes[m,"mod_b"] <- temp[grepl(":", rownames(temp)),1]
  df_ame_bayes[m, "mod_se"] <- temp[grepl(":", rownames(temp)),2]
...
df_ame_bayes <- df_ame_bayes %>%
  mutate(mod_var = ifelse(grepl("noconf_govZ", Model, fixed = T), "B_No_confidence", 
         ...
         estimation = ifelse(grepl("lmer", df_ame_bayes$Model), "GLM", "OLS"),
