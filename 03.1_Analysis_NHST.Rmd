...
```{r macro_corrs}
macro_corr <- df_macro %>% 
  dplyr::select(gov_redist, incdiff_large, corrupt_top, corr, 
...
We want to model the relationship within countries only, to try and isolate the moderating effect of individual attitudes. Although it is likely, if not certain that context shapes attitudes and attitude formation processes, we 'force' all contextual factors to work through the individual's perception of corruption. 

```{r ilevel}

m00 <- lmer(gov_redist ~ female + educyrs + incdiff_large_w*corrupt_top_w + (1|cyear), data = df_na)
```


### All Plausible Country-Level Models
...
colnames(df_ame_bayes) <- c("Model", "mod_b", "mod_se")
count <- 1
for (m in model_list_bayes) {
  ...
  df_ame_bayes[count,"Model"] <- paste(as.character(f[2]), as.character(f[1]), as.character(f[3]))
