...
ivs <- readRDS(here::here("data","ivs_aggregated.rds")) %>%
  mutate(iso3c = COUNTRY_ALPHA,
         wave = case_when(
           ...
           TRUE ~ NA_real_)) # fit years of WVS/IVS into nearest wave of ISSP 1987, 1992, 1999, 2009, 2019


ivs <- ivs %>% # create an interpolated version of ivs variables that takes the mean by country to fill in missing - most countries do not change much over time
  group_by(iso3c) %>%
...
# one WVS/IVS with original values, one with imputed values (different merge variables)
ivs_2 <- ivs %>%
  dplyr::select(iso3c, year, liberal_ix, liberal_redist_ix, confidence_gov_ix)

ivs <- ivs %>%
  dplyr::select(-c(year, liberal_ix, liberal_redist_ix, confidence_gov_ix))
...
df <- df %>%
  left_join(ivs, by = c("iso3c", "wave")) %>%
  left_join(ivs_2, by = c("iso3c", "year")) %>%
...
df <- df %>%
  ...
  dplyr::select(-c(confidence_gov_ix, liberal_ix, liberal_redist_ix))
...
df_macro <- df_macro %>%
  mutate(liberal_all = ifelse(!is.na(liberal), liberal, liberal_i),
         ...
         liberal_low = ifelse(iso3c %in% c("HRV","CYP","ITA","SVN","CZE"), NA, liberal_low),
         gdp_di = ifelse(!is.na(liberal_high), 1, 0)) # remove some questionable cases
...
df_macro_m <- df_macro %>%
  dplyr::select(iso3c, wave, cpiZ, cpiZ_i, libZ, libZ_i, gdp_di, gdp_pc_10k, gini_disp, socx, noconf_govZ, noconf_govZ_i)
