...
wvs_2 <- wvs %>%
  dplyr::select(iso3c_wave, liberal_ix, liberal_redist_ix, confidence_gov_ix)

wvs <- wvs %>%
  dplyr::select(-c(iso3c_wave, year, liberal_ix, liberal_redist_ix, confidence_gov_ix))
...
df_na <- left_join(df_na, wvs, by = c("iso3c", "wave"))
df_na <- left_join(df_na, wvs_2, by = "iso3c_wave")
...
m_conf <- lm(confidence_gov_i ~ gdp_pc_10k + socx + cpi_ii, data = df_macro)
