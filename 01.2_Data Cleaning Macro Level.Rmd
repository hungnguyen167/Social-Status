...
wvs <- wvs %>% # create an interpolated version of wvs variables that takes the mean by country to fill in missing - most countries do not change much over time
  ...
  mutate(liberal_ix = mean(liberal, na.rm = T),
         ...
         liberal_i = ifelse(is.na(liberal), liberal_ix, NA),
         liberal_redist_i = ifelse(is.na(liberal_redist), liberal_redist_ix, NA),
         confidence_gov_i = ifelse(is.na(confidence_gov), confidence_gov_ix, NA)) %>%
  select(-c(liberal_ix, liberal_redist_ix, confidence_gov_ix)) %>%
  ungroup() 

# one WVS with original values, one with imputed values (different merge variables)
wvs_2 <- wvs %>%
  select(iso3c, year, liberal_i, liberal_redist_i, confidence_gov_i)

wvs <- wvs %>%
  select(-c(year, liberal_i, liberal_redist_i, confidence_gov_i))
...
df_macro <- df_na %>%
  select(iso3c, wave, gov_redist, incdiff_large, gini_disp, gini_mkt, socx, socx_perhead, cpi15_20, liberal, liberal_i, liberal_redist, liberal_redist_i, confidence_gov, confidence_gov_i, democratic, gdp_pc_10k, regime, gap_no_gov, gov_redist_agree, incdiff_large_agree) %>%
  group_by(iso3c, wave) %>%
  summarise_all(mean, na.rm = T) %>%
  ungroup()
