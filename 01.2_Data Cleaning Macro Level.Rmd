...
df_na <- df_na %>%
  ...
  mutate(incdiff_b = mean(incdiff, na.rm = T),
         ...
         corrupt_top_b = mean(corrupt_top, na.rm = T)) %>%
  ungroup() %>%
  ...
  mutate(incdiff_w = incdiff - incdiff_b,
         ...
         democratic = regime,
         corrupt_top_w = corrupt_top - corrupt_top_b)
