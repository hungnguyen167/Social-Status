...
wvs <- wvs %>%
  group_by(iso3c, year) %>%
  summarise_all(mean, na.rm = T) %>%
  dplyr::select(-c(wave, iso3c_wave, liberal_ix, liberal_redist_ix, confidence_gov_ix))

df_na <- left_join(df_na, wvs, by = c("iso3c", "year"))
