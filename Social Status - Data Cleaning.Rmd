...
A. HUNG - can you integrate nt87 and ie87 into the socin file please? I've added the data files, but not recoded them yet
   ...
   HUNG - careful! in the env93 module you recoded all values of the ISCO variable into ISCO68 and then converted them to 88, but many countries already were in isco88 so this created a lot of mistakes. Always check variables after recoding them. For example there should be an equal number of non-missing values for isco68 and isco88 after conversion. I fixed it in the env93 section, but left your original code there in case you wanted to see it, just search for "PROBLEM CODE", it is in two places (and of course delete it after please)
   
   HUNG - we need to get spouse isco and spouse isco88_2digit, can you please add these everywhere? please name them spisco88 and spisco88_2digit. Of course for many countries there will be missing spisco88 in the earlier surveys, but spouse isco will be really important in determining the status of a housewife/partner.
...
nt87 <- read_dta(paste(wd, "ZA1680.dta", sep=""))
nt87 <- subset(nt87, nt87$v3 == 7)
...
socin <- socin %>%
  mutate(isco88_2digit = ifelse(!is.na(isco88), trunc(isco88/100), NA))
```
...
socin <- dplyr::select(socin, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88, isco88_2digit)
...
dk99 <- dplyr::select(dk99, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88, isco88_2digit)
...
ie99 <- dplyr::select(ie99, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88, isco88_2digit)
...
nl09 <- dplyr::select(nl09, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88, isco88_2digit)
...
# PROBLEM CODE
# This is a mistake, only some countries have ISCO68, not all. The code that appears later does not fix it, it just makes everything NA, but its too late then.
...
# In Germany, Norway, Czech Republic, BUlgaria, Canada, Israel, New Zealand, and Spain, isco68 appears to be 3-digit, multiply by 10 and add 1.


env93$isco68 <- ifelse(env93$cntry == 276 | env93$cntry == 100 | env93$cntry == 124 | env93$cntry == 376 | env93$cntry == 554 | env93$cntry == 578 | env93$cntry == 705 | env93$cntry == 724, (env93$isco68 * 10)+1, env93$isco68)
env93$fisco68 <- ifelse(env93$cntry == 276 | env93$cntry == 100 | env93$cntry == 124 | env93$cntry == 376 | env93$cntry == 554 | env93$cntry == 578 | env93$cntry == 705 | env93$cntry == 724, (env93$fisco68 * 10)+1, env93$fisco68)
env93$misco68 <- ifelse(env93$cntry == 276 | env93$cntry == 100 | env93$cntry == 124 | env93$cntry == 376 | env93$cntry == 554 | env93$cntry == 578 | env93$cntry == 705 | env93$cntry == 724, (env93$misco68 * 10)+1, env93$misco68)
...
# PROBLEM CODE
# it is too late here to do this, we need those isco 88 scores for these countries, not NA!
...
env93 <- env93 %>%
  mutate(isco88_2digit = ifelse(!is.na(isco88), trunc(isco88/100), isco88_2digit)) # Truncate isco88 values

# Select relevant variables 

env93 <- dplyr::select(env93,c(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, v231, v253, isco88, isco88_2digit, spisco88_2digit))
...
env00 <- env00 %>%
  mutate(isco88_2digit = ifelse(!is.na(isco88), trunc(isco88/100), NA)) # Truncate isco88 values

# Select relevant variables 
env00 <- dplyr::select(env00,c(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88, isco88_2digit)) 
```
...
# Select relevant variables 
env10 <- dplyr::select(env10,c(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88, isco88_2digit))
