...
A. ISCO in env93 is coded in mixed ways. I have listed them at the top of the code chunk (copied from the code book). First, the standard isco68 countries are in *3-digit ISCO* so they need to be multiplied by 10 before converting to isco88. Meanwhile some other countries are already in isco88 so they need to be directly transfered to the isco88 variable without transformation.

B. Need to use the supplementary file to divide East and West Germany at some point
...
temp <- lapply(temp, function(x) ntile(x, 10)) # deciles
...
#### 1987
AUS - ASCO1
DEU, AUT, CHE, HUN - ISCO68
GB - KOS 1980
US - Census 1970 codes
POL - handcodes [
01 Professionals
02 Managers
03 Semiprofessionals white-collar
04 Office workers
05 Service workers manual/non-manual
06 Owners of manufacturing, trades, artisans
07 Foremen
08 Skilled manual
09 Semi-skilled manual
10 Unskilled manual
11 Farmers and family workers
12 Agricultural laborers employed by farms
]
NET - handcodes [
...
ITA - handcodes [
*Entrepreneur*
011 Entrepreneuers, manufacturers
012 Independent professionals
013 Authors, writers artists
014 Proprietors, owners,drawer of rents, living from revenue
*Professionals*
021 Managers, officials, civil servants, administrators
022 Professors, teachers
023 Journalists
024 Officers and non-commisioned officers in pro army
...
034 Day labor, farm, wage
*Labourers, indep/self-emp*
041 Retail traders, merchants, salesment (with trade/shop)
042 Trade agents, sales reps (self-emp)
043 Craftsmen
044 Other self-emp without shop
045 Family assistants
046 Farmers, farm managers
...
AUT, CAN, DEU, NET, BG, CS, USA, HUN - ISCO68
AUS, POL, SLO, NZ, RUS - ISCO88
...
# Recoding to isco88_2: Australia 1987
# Data are in ASCO First Edition, emailed Kelley and Evans 10-Jun-20 to ask for correspondance tables

# Here is Nate's code to create rough ISCO88-2 categories
socin <- socin %>%
  mutate(
    isco88_2digit = case_when(
...
# Recoding to isco88_2digit: Italy 1987/Italy 1992 (not found)


# Recoding to isco88_2digit: Great Britain 1987/1992 (not found)


# Recoding to isco88_2digit: Poland 1987 (manually code to EGP6?)


# Recoding to isco88 & isco88_2digit: U.S 1987
...
socin <- dplyr::select(socin, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88)
...
dk99 <- dplyr::select(dk99, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88)
...
ie99 <- dplyr::select(ie99, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88)
...
nl09 <- dplyr::select(nl09, cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88)
...
# In Germany, Norway, Czech Republic, BUlgaria, Canada, Israel, New Zealand, and Spain, isco68 appears to be 3-digit, multiply by 10.


env93$isco68 <- ifelse(env93$cntry == 276 | env93$cntry == 100 | env93$cntry == 124 | env93$cntry == 376 | env93$cntry == 554 | env93$cntry == 578 | env93$cntry == 705 | env93$cntry == 724, env93$isco68 * 10, env93$isco68)
env93$fisco68 <- ifelse(env93$cntry == 276 | env93$cntry == 100 | env93$cntry == 124 | env93$cntry == 376 | env93$cntry == 554 | env93$cntry == 578 | env93$cntry == 705 | env93$cntry == 724, env93$fisco68 * 10, env93$fisco68)
env93$misco68 <- ifelse(env93$cntry == 276 | env93$cntry == 100 | env93$cntry == 124 | env93$cntry == 376 | env93$cntry == 554 | env93$cntry == 578 | env93$cntry == 705 | env93$cntry == 724, env93$misco68 * 10, env93$misco68)
...
# Australia, (Slovenia), Poland, Russia, Ireland: ISCO88 
env93[which(env93$cntry %in% c(36,705,616,643,372)),]$isco88 <- NA 
env93[which(env93$cntry %nin% c(36,705,616,643,372)),]$isco88_2 <- NA 

env93[which(env93$cntry %in% c(36,616,643,372)),]$fisco88 <- NA 
env93[which(env93$cntry %nin% c(36,616,643,372)),]$fisco88_2 <- NA 

env93[which(env93$cntry %in% c(36,616,643,372)),]$misco88 <- NA 
env93[which(env93$cntry %nin% c(36,616,643,372)),]$misco88_2 <- NA 
...
env93 <- dplyr::select(env93,c(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, v231, v253, isco88))
...
# Select relevant variables 
env00 <- dplyr::select(env00,c(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88)) 
```
...
# Select relevant variables 
env10 <- dplyr::select(env10,c(cntry, year, wave, age, ageC, female, educyrs, degree, child, married, rincZ, hincZ, isei, egp, egp6, f_egp, f_egp6, f_isei, m_egp, m_egp6, m_isei, union, sparent, wrkhrs, wrkst, incdiff, isco88))
...
tab isco88_2d if groups==. //Check whether all ISCO groups are used 
/* Three occupational groups are omitted: 0-3 (Armed Forces), 14, 44.
Armed Forces are supposed to be missings; 44 was put into class group 1 (Oesch class 10) and 44 into class group 2 (Oesch class 11)*/
...
# create ISCO88 2-digit


df$isco88_2d <- ifelse(df$isco88 < 1000, NA, df$isco88)
df$isco88_2d <- round(df$isco88_2d/100, 0)
# create Schwander and Haeusermann coding groups

df <- df %>%
  mutate(sh_groups = case_when(
    isco88_2d==11 | isco88_2d==12 | isco88_2d==13 | isco88_2d==21 | isco88_2d==22 | isco88_2d==24 | isco88_2d==34 ~ 1,
    isco88_2d==31 | isco88_2d==41 | isco88_2d==42 ~ 2,
    isco88_2d==60 | isco88_2d==61 | isco88_2d==70 | isco88_2d==71 | isco88_2d==72 | isco88_2d==73 | isco88_2d==74 | isco88_2d==80 | isco88_2d==81 | isco88_2d==82 | isco88_2d==83 | isco88_2d==92 | isco88_2d==93 ~ 3,
    isco88_2d==23 | isco88_2d==32 | isco88_2d==33 ~ 4,
    isco88_2d==51 | isco88_2d==52 | isco88_2d==91 ~ 5)
  )
