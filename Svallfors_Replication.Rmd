---
title: "Attitudes toward Redistribution Thirty Years After: Theoretical and Empirical Reflections on Svallfors’ (1997) Findings"
output:
...
In the following we replicate the operationalization of the independent variable of Svallfors (1997) regarding attitudes to redistribution. 

We use the ISSP 1992 'Social Inequality' data provided by GESIS. File (https://www.gesis.org/issp/modules/issp-modules-by-topic/social-inequality/1992 "ZA.2310.dta").
...
**Savllfors Original Country Cases:** Swe, Nor, Ger, Aut, Aus, Nz, Can, USA
...
pacman::p_load("dplyr","tidyverse","knitr","haven","car","plyr","data.table","expss","htmlTable","magrittr","ltm","questionr","survey","remotes","stargazer","lavaan","ltm", "tidylog")
...
This table uses the dplyr::mutate function to obtain weighted means then tabulates.
...
This version uses sapply with a function to generate weighted means.
...
For starters we look at the correlations between the attitude variables and a few criterion variables.

```{r measurement2, echo = T, include = T}
# Correlation commands are easier when the data.frame has only the variables of interest.

socIn92cor <- as.data.frame(dplyr::select(socIn92recode, v57, v59, v62, r1p, r2p, r3p, v3, v99, v100, v102))

# Make 'agree' = higher values
car::recode(socIn92cor$v57,recodes = '1=5; 2=4; 3=3; 4=2; 5=1')
car::recode(socIn92cor$v59,recodes = '1=5; 2=4; 3=3; 4=2; 5=1')
car::recode(socIn92cor$v62,recodes = '1=5; 2=4; 3=3; 4=2; 5=1')
...
knitr::kable(cor,format="html", digits = round(2), col.names = c("IncD_5","Jobs_5","BasicI_5","IncD_3","Jobs_3","BasicI_3","Country","Male","Age","Ed_Years"), caption = "Table A. Correlations of Original and Recoded ISSP Questions from Svallfors (1997)")
```

# Now prep data for SEM, separate data.frames for each.
...
# This model performs a factor analysis using Svallfors' coding of the three items (3-categories). Agree / Neutral / Disagree.
m_sval <- 'att_sval =~ r1p + r2p + r3p' 

# This model reflects Savllfors' actual scaling with each item's factor loading fixed at 1. The 1 determines that each has equal weight in constructing the scale. Any additive index automatically assigns equal weight to every item in the scale.
m_sval1 <- 'att_sval1 =~ 1*r1p + 1*r2p + 1*r3p' # 1* = fixed at 1

# This model uses the original coding of the variables (5-categories). Strongly agree / Agree / Neutral / Disagree / Strongly disagree
m_norm <- 'att_norm =~ v57 + v59 + v62'


# The 'cfa' command is known as a "confirmatory factor analysis". This simply means that we assign the factor structure to the model, rather than having the model do an "exploratory factor analysis" where it tests different possible factor structures among the various combinations.
