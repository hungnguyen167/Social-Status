...
Having created/recoded theses 4 variables, we replicate table 1 and table 2 of his study.
...
knitr::kable(table1,format="html", digits = round(1),col.names = cnt, caption = "Table 1. Attitudes to redistribution in eight nations. Percentage agreeing with certain propositions")
...
table2<-as.data.frame(rbind(govIndexmean,govIndexsd))
table2<-table2[,c(6,5,2,4,1,7,8,3)]

item.table2<-c("Mean","Standard Deviation")
row.names(table2)<- item.table2
colnames(table2)<- cnt #make the same way for table 1! table -as r object- is already more complete this way :)

#cronbach's alpha is still missing here 
