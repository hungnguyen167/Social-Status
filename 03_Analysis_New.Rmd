...
#### Not for specification curve, no interaction
...
for (m in 1:length(model_list)) {
  ...
  temp <- as.data.frame(get(mn)[["coefficients"]])
  
  temp <- temp[grepl('cpiZ|noconf_govZ|libZ|incdiff_large', rownames(temp)), 1:2]
...
```{r ameclean}
df_ame <- df_ame %>%
  mutate(mod_var = ifelse(grepl("noconf_govZ", Model, fixed = T), "No_confidence", 
                   ifelse(grepl("cpiZ", Model), "Corruption_percept", "Liberal_values")),
