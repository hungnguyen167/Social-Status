...
df_macro$cpi15_20_i <- predict.lm(m_cpi, newdata = df_macro)

df_macro <- df_macro %>%
  mutate(cpi15_20_i = -1*cpi15_20_i + 100, # reverse scale, higher values more corrupt percept
         cpiZ = scale(cpi15_20_i))

# make high and low corruption perceptions
df_macro <- df_macro %>%
  mutate(cpi_low = ifelse(!is.na(liberal_low), cpiZ, NA),
...
```{r prep_data}
df_na <- df_na %>%
  select(-c(gini_disp, socx)) 

df_macro <- as.data.frame(df_macro)

df_macro_m <- df_macro %>%
  select(iso3c, wave, cpiZ, liberal_i, gdp_pc_10k, gini_disp, socx, confidence_gov_i_i, cpi15_20_i)
...
tab_conf <- mod_models(df_na, "gov_redist", "incdiff_large", "confidence_gov",dv.labels = c("M1", "M2", "M3","M4"),
           pred.labels = c("Intercept", "incdiff_large", "conf", "incdiff_large*conf")) ## medium level of abstraction
tab_lib <- mod_models(df_na, "gov_redist", "incdiff_large", "liberal_redist",dv.labels = c("M1", "M2", "M3","M4"),
...
```{r cor_1}
df_libvars <- df_na[,c("cpiZ", "confidence_gov", "liberal_redist")]
...
```{r cor_2}
df_cvars <- df_na[,c("cpiZ", "confidence_gov", "liberal_redist","regime", "socx", "gdp_pc_10k")]
cor_mat_2 <- cor(df_cvars, use = "complete.obs")
round(cor_mat_2, 2)
```

M03 models with control variables. Regime and gdp_pc_10k are highly correlated. One would cancel out the effect of another. 

```{r wc}
mwc_01 <- lmer(gov_redist ~  1 + incdiff_large + cpiZ + cpiZ*incdiff_large  + 
                  (1 | iso3c) + (1 | wave) + regime + socx, data = df_na) 
mwc_02 <- lmer(gov_redist ~  1 + incdiff_large + confidence_gov + confidence_gov*incdiff_large  + 
                  (1 | iso3c) + (1 | wave) + regime + socx, data = df_na) 

mwc_03 <- lmer(gov_redist ~  1 + incdiff_large + liberal_redist + liberal_redist*incdiff_large  + 
                  (1 | iso3c) + (1 | wave) + regime + socx, data = df_na) 
```

Print out the table
```{r}
tab_wc <- tab_model(mwc_01, mwc_02,mwc_03)
knitr::knit_print(tab_wc)
```

Let's see how it would work with a Bayesian setting
...
marg_df$cpiZ <- c(-2,-1,0,1,2)
marg_df$incdiff_large_3 <- c(3,3,3,3,3)
marg_df$incdiff_large_4 <- c(4,4,4,4,4)
marg_df$b_incd <- c(0.58,0.58,0.58,0.58,0.58)
marg_df$b_cpiZ <- c(0.32,0.32,0.32,0.32,0.32)
marg_df$b_incXc <- c(-0.08,-0.08,-0.08,-0.08,-0.08)
