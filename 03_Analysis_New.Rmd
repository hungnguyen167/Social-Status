...
m00 <- lmer(gov_redist ~ 1 + incdiff_large + (1 | iso3c) + (1 | wave), data = df_na) # baseline
m01 <- lmer(gov_redist ~ 1 + incdiff_large + cpiZ + (1 | iso3c) + (1 | wave), data = df_na) # with main effects
m02 <- lmer(gov_redist ~ 1 + incdiff_large + cpiZ + (1 | iso3c) + (incdiff_large | wave), data = df_na) # random-slopes
m03 <- lmer(gov_redist ~ 1 + incdiff_large + cpiZ + cpiZ*incdiff_large  + (1 | iso3c) + (1 | wave), data = df_na) # moderation

```

```{r mod_tab}
tab_model(m00, m01, m02, m03, show.ci = F, show.aic = T, show.loglik = T, show.icc = T, p.style = "stars")
```
