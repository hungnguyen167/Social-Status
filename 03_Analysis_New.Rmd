...
mod_models <- function(df, dv, main_iv, value_var, add_vars=NULL, return_tab = TRUE, ...) {
    if (is.null(add_vars)){
        df_new <- df %>%
        ...
        df_new <- df %>%
          select(iso3c, wave, dv, main_iv, value_var, all_of(add_vars))
    }
    ...
    ## moderation with additional variables
    if (!is.null(add_vars)) {
        m04 <- lmer(get(dv) ~  1 + get(main_iv) + get(value_var) + get(value_var)*get(main_iv)  + 
                  (1 | iso3c) + (1 | wave) + ., data = df_new)
         tab_temp <- tab_model(m00, m01, m02, m03, m04, show.ci = F, show.aic = T, 
...
tab <- mod_models(df_na, "gov_redist", "incdiff_large", "cpiZ")
```

Try three different "value" variables

```{r}
tab_cpi <- mod_models(df_na, "gov_redist", "incdiff_large", "cpiZ")
tab_liberal <- mod_models(df_na, "gov_redist", "incdiff_large", "liberal_redist")
tab_confidence <- mod_models(df_na, "gov_redist", "incdiff_large", "confidence_gov")
