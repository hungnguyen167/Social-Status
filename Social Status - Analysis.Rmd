...
mod_string3 <- " model {
  ...
  zbeta0 ~ dnorm((1+5)/2,1/5^2)
  zbeta1 ~ dnorm(0,1/5^2)
  sigma ~ dunif(5/1000,5*10)
  for (i in 2:3) {c[i] ~ dnorm(i+0.5, 1/2^2)}
}"
...
data_jags3 <- list(y = as.factor(df_new$reduce), zx = as.factor(df_new$sh_groups_new), N = nrow(df_new), c = c)

mod3 <- jags.model(textConnection(mod_string3), data = data_jags3, n.chains=3)

params3 = c("zbeta0", "zbeta1", "sigma", "c")
