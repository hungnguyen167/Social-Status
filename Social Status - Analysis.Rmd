...
library(pacman)
pacman::p_load("tidyverse","knitr","car","expss","stargazer","lavaan","kableExtra","MASS","glm.predict","brant", "semTools","ggplot2","countrycode","rjags")
...
pacman::p_load(readr)
cwed <- read.csv("https://raw.githubusercontent.com/hungnguyen167/Social-Status/master/cwed-subset.csv?token=AG6CBQYFHE3RYFAXHOMKPTS7IQGHU")
...
gini <- aggregate(df, by = list(df$cntry), function(x) Gini(x))
gini <- select(gini, Group.1, rincO)
gini <- rename(gini, cntry = Group.1)
gini$rincO <- round(gini$rincO, digits = 3)
...
de10 <- de10[complete.cases(de10$sh_groups_new),]  # get complete cases for the model to work
...
Let's try a more complicated model using Bayesian estimation, this time an Ordinal Logistic Regression
...
mod_string2 <- " model {
  for ( i in 1:n) {
    ...
    for ( k in 2:(n-1) ) {
      pr[i,k] <- max( 0, pnorm(thresh[k], mu[i], 1/sigma^2) - pnorm(thresh[k-1], mu[i], 1/sigma^2))
    }
  
    pr[i,n] <- 1 - pnorm(thresh[n-1], mu[i], 1/sigma^2)
    mu[i] <- b0 + b1*x[i]
  }
  b0 ~ dnorm((n+1)/2, 1/n^2)
  b1 ~ dnorm(0, 1/n^2)
  sigma ~ dunif(n/1000, n*10)
  for ( k in 2:(n-2)) {
    thresh[k] ~ dnorm(k+0.5, 1/2^2)
  }
}"
thresh = rep(NA, nrow(de10)-1)
thresh[1] = 1+0.5
thresh[nrow(de10)-1] = nrow(de10)-0.5
data_jags2 = list(y = as.factor(de10$reduce), x = as.numeric(de10$sh_groups_new),n = nrow(de10), thresh = thresh)

params = c("mu","sigma")

mod2 = jags.model(textConnection(mod_string2), data = data_jags2, n.chains = 2)
