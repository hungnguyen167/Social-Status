...
tab2 <- dplyr::select(tab2, year, ageC, female, educyrs, child, married, rincZ, hincZ, isei, egp_un, egp_skl, egp_othr, egp_rou, egp_svc2, egp_svc1, sh_groups_1:sh_groups_5, wk_full:wk_other, wrkhrs, union, sparent, reduce)
...
fig2 <- dplyr::select(fig2, year, cyear, lab, reduceM, reduceSE, wave, cntry, sh_groups)

fig2 <- aggregate(fig2,  by=list(fig2$cyear,fig2$sh_groups), FUN=mean, na.rm=T)

fig2de <- subset(fig2, cntry == 276)
fig2de$sh_groups <- as.factor(fig2de$sh_groups)

ggplot(fig2de, aes(x=year, y=reduceM, color=sh_groups)) + 
    geom_errorbar(aes(ymin=reduceM-reduceSE, ymax=reduceM+reduceSE), colour="grey", width = .15) +
    geom_point(size=2) +
    geom_line() +
...
fig2gb$sh_groups <- as.factor(fig2gb$sh_groups)

ggplot(fig2gb, aes(x=year, y=reduceM, color=sh_groups)) + 
    geom_errorbar(aes(ymin=reduceM-reduceSE, ymax=reduceM+reduceSE), colour="grey", width = .15) +
    geom_point(size=2) +
    geom_line() +
    labs(title = "Support for Redistribution Over Time, Germany") +
...
fig2us$sh_groups <- as.factor(fig2us$sh_groups)

ggplot(fig2us, aes(x=year, y=reduceM, color=sh_groups)) + 
    geom_errorbar(aes(ymin=reduceM-reduceSE, ymax=reduceM+reduceSE), colour="grey", width = .15) +
    geom_point(size=2) +
    geom_line() +
    labs(title = "Support for Redistribution Over Time, Germany") +
...
de10 <- completeFun(de10, "reduce")
de10$groups <- as.factor(de10$groups)
...
de10 <- de10 %>%
  mutate(group1 = ifelse(groups == 1, 1, 0),
         group2 = ifelse(groups == 2, 1, 0),
         group3 = ifelse(groups == 3, 1, 0),
         group4 = ifelse(groups == 4, 1, 0)
  )
