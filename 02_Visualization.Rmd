...
mmmPOOLED <- ggplot() + geom_smooth(data=df, aes(x=incdiff_large, y=gov_redist, color = as.character(wave)), size=1, se=F, method="lm", linetype=0) +
  geom_smooth(data=df, aes(x=incdiff_large, y=gov_redist, color = "Pooled"),se=T, size=1, method="lm") +
