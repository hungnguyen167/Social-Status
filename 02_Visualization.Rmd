...
ggarrange(Fig2.1, Fig2.1_redist, Fig2.2, widths = c(1.1,1,1.1), nrow = 1)
dev.off()
...
m01 <- lm(gov_redist ~ incdiff_large, data = df_na) # the regression from the equation above
summary(m01)
```
...
```{r tbl_statt_map}
agg_png(filename = here::here("results","statt_plot.png"), width = 1500, height = 1200, res = 144)
df_tbl <- df_macro %>%
...
#old plot
ggplot(df_tbl, aes(x = gov_redist_agree ,
                   y = incdiff_large_agree)) + 
  geom_point(aes(size = gap_no_gov, color=gap_no_gov),
             alpha = 0.75) + 
  geom_text(aes(label=iso3c), 
            position = "dodge",
            vjust=0.3, hjust=-0.2)+
  scale_size(guide = "none") +
  scale_color_continuous(limits = range(0:40),
                         type = "viridis",
                         labels = scales::percent_format(scale = 1),
                         guide = guide_colorbar(title = "Income gap is too large,\ngov't is not responsible",
                               title.position = "left",
                               title.vjust = .8, 
                               frame.colour = "black", 
                               ticks.colour = "black")) +
  labs(x = "Government should redistribute",
       y = "Differences in income is too large") +
  ...
  scale_x_continuous(labels = scales::percent_format(scale = 1),
                     breaks = c(40,50,60,70,80,90,100), 
                     limits=c(40,100)) + theme(axis.line = element_line(size = 1, colour = "black", linetype=1))
#new plot
ggplot() + 
  geom_point(data = df_tbl, aes(x = incdiff_large_agree , y = gap_no_gov, color=iso3c),alpha = 0.75,size=4) + 
  geom_text(data = df_tbl, aes(x =  incdiff_large_agree  , y =gap_no_gov,label=iso3c), 
            position = "dodge",
            vjust=0.3, hjust=-0.2)+
  labs(x = "Percentage who agree vs disagree \n that incomegap is too large",
       y = "Percentage who agree government\n should redistribute income out \n of those who agree income gap is too large") +
  ...
  scale_x_continuous(labels = scales::percent_format(scale = 1),
                     breaks = c(40,50,60,70,80,90,100),  
                     limits=c(70,100)) + 
  theme(axis.line = element_line(size = 1, colour = "black", linetype=1)) + scale_colour_viridis_d(guide="none") 
